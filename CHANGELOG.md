...
- **`00-inbox/TASKS.md` and `00-inbox/COG-DASHBOARD.md` now ship with the fork.** Both are pure query pages — Tasks-plugin blocks (Overdue / Due Today / This Week / No Date / By Project) and Dataview blocks over COG's own folder paths and frontmatter types — so they work in any vault and contain nothing personal. Neither existed on a fresh install before: upstream has no reference to `TASKS.md` at all, and nothing in either project ever created a dashboard, which left **`/task-triage` targeting a file that never existed**. `cog-install.ps1` gains the **Dataview** plugin (`dataview`, `isDesktopOnly: false`) so the dashboard renders rather than showing raw code blocks. Onboarding still creates `MY-PROFILE.md`, `MY-INTERESTS.md` and `MY-INTEGRATIONS.md` as before.
- **`gemini-scribe/` is excluded from the vault index.** The installer merges `userIgnoreFilters` into `.obsidian/app.json`, keeping the assistant's chat sessions and generated skills mirror out of search, the graph and unlinked mentions, and demoting them in the quick switcher. The merge preserves any settings Obsidian already wrote and is idempotent.
- **Mobile sync moved from Remotely Save to [OneDrive Sync](https://github.com/JeffSteinbok/obsidian-onedrive)** (`onedrive-sync`). Remotely Save's last public release is `0.5.25` from 2024-10-20 with 222 open issues, and its free tier can only reach OneDrive's `/Apps/remotely-save` folder — which would have meant a second copy of the vault in the same account, silently diverging from the real one. OneDrive Sync is actively released (`2.0.0`, 2026-08-10), supports iOS and Android, and in Full Access mode syncs the vault where it already lives. **Setup must set the sync folder to `/cog-second-brain`:** with Full Access and an empty folder the delta query falls through to `/me/drive/root/delta` and walks the whole account (`src/api/oneDriveClient.ts:612-622`), so `BFU-SETUP.md` makes the folder step explicit and adds a post-sync check.

### Removed
- **The sync plugin is no longer installed on desktops.** `cog-install.ps1` ships three plugins (Gemini Scribe, Tasks, Calendar). The desktop vault is synced by the OneDrive client itself; a second syncer over the same folder only adds conflicts. Phones install OneDrive Sync by hand, which is unavoidable anyway since Android plugin settings cannot be scripted.
