...
| `/playwriter` | Drive the user's own Chrome for JS-heavy, login-walled, or lazy-loaded pages | "open this in my browser", "log in and get" |
| `/obsidian` | Shared reference: vault operations, frontmatter rules, tags, tasks, Obsidian markdown syntax. Read by other skills, not usually invoked directly | (consulted by skills that touch the vault) |
| `/loop-engineering` | Shared reference: the agent loop, deterministic verifiers, termination conditions, named iteration patterns. Read when designing or debugging an iterative skill | (consulted when building a skill that iterates) |

## Daily Journal

Ambient behavior, not a command. The trigger lives here because it must load in every session; the procedure lives in `.agents/skills/daily-journal/SKILL.md`.

- **After finishing a meaningful unit of work, append one entry** to `01-daily/journal/YYYY-MM-DD.md`. Get the date from the system, never from inference. Meaningful = shipped or committed something, produced a deliverable, made a decision, changed direction, or hit a notable blocker.
- **Create the file from the skill's template on the day's first entry.** Append only, newest at the bottom of `## Log`.
...
Composition rules for everything the agent writes. These hold regardless of which output style is active, or whether one is active at all - an output style controls presentation, this section controls substance.

Optimize for information gain, not apparent completeness. The failure mode is framework slop: ordinary reasoning dressed up as a consulting memo.

- **No invented frameworks.** Do not name reasoning as a framework, gate, layer, pillar, lens, or numbered taxonomy unless it exists in the source material or the grouping genuinely cuts complexity. Three unrelated points are a list, not three pillars.
- **Headings name subject matter, never rhetorical function.** "Authentication", not "Why this matters", "The key insight", "What this is not", "The bottom line", "The deeper point", "The uncomfortable truth".
- **State the thing before what it is not.** Delete the negative runway.
- **No straw-man corrections.** "It's not X, it's Y", "This isn't about X", "While it may seem..." are allowed only when X is a position someone relevant actually holds.
- **Space proportional to evidence and consequence.** One finding may deserve ten paragraphs and the next one line. Equal-sized sections are a tell.
