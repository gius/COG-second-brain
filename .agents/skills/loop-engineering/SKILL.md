...
description: >
  Shared loop-engineering reference for COG skills — the agent loop, deterministic verifiers,
  termination conditions, in-loop context management, and named patterns. Invoke when designing or
  debugging a skill that iterates (search-verify-retry, scan-until-dry, fetch-retry-gate).
metadata:
  roles: all
  keywords: loop,iterate,verifier,termination,retry,orchestrator,worker,react,reflexion,agent-loop
  display-name: Loop Engineering
  internal: true
---
...
This is a reference and design aid, not a content-generating workflow. Skills that loop (daily-brief, knowledge-consolidation, url-dump, weekly-checkin, and research/triage skills like auto-research, scout, and task-triage) link here instead of restating the rules. Invoke it directly when you are building or fixing an iterative skill.
...
| **Orchestrator-workers** | split into subtasks, run in fresh windows, synthesize | team-mode scans, auto-research threads, daily-brief team mode |
| **Loop-until-dry** | keep going until K passes in a row surface nothing new | knowledge-consolidation theme extraction |
| **Human-in-the-loop** | escalate or ask when the loop is stuck or the call is the user's | weekly-checkin reflection, task-triage needs-user, onboarding |
