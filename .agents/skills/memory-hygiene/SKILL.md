...
metadata:
  roles: all
  keywords: memory hygiene,audit my memories,stale memories,memory sweep,verify memories,drifted memory,memory audit,trust sweep
  display-name: COG Memory Hygiene
---
...
### Out of scope for stamping: frameworks and patterns

Files in `05-knowledge/consolidated/` and `05-knowledge/patterns/` carry their own trust signal — the `status:` field (`emerging` | `working` | `stable`), owned by `/knowledge-consolidation`. Do **not** add `last_verified` or `confidence` to them; two competing trust signals on one file is worse than one.

Environment-dependent claims *inside* those files are still in scope: verify them and fix drifted facts in place, exactly as for any other note. Only the frontmatter stamping is withheld. Report the fixes as normal.
...
Skip stamping entirely for frameworks and patterns (see Scope).
